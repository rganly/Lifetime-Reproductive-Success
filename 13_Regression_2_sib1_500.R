...
ep <- ep[ ,c("KANTAHENKILON_TNRO","n_child","n_gchild","SUKULAISEN_SYNTYMAPV","SUKUPUOLI","SYNTYMAKOTIKUNTA","ISCED97","ISCED97.Parent","EduYears","EduYears.Parent",endpoints[192:500])]
...
sink(file="popsib_reg_selected_endpoint_1_500.output", append=T, type="output", split=T)
...
for (ep_n in 192:500){
	...
	for (sex_n in 1:2){	
		...
		if (ep_n==1 & sex_n==1)	{		
			write.table(res, "popsib_reg_selected_endpoint_1_500_qsub.tsv", append=F, quote=F, sep=" ", row.names=F, col.names=T)
		} else {
			write.table(res, "popsib_reg_selected_endpoint_1_500_qsub.tsv", append=T, quote=F, sep=" ", row.names=F, col.names=F)		
		}
	}
