...
sink(file="pop_reg_selected_endpoint_1_500_qsub.output", append=F, type="output", split=T)
...
for (ep_n in 1:500){
	...
	for (sex_n in 1:2){	
		...
		if (ep_n==1 & sex_n==1)	{		
			write.table(res, "Population_based_regression_endpoint_1_500_qsub.tsv", append=F, quote=F, sep=" ", row.names=F, col.names=T)
		} else {
			write.table(res, "Population_based_regression_endpoint_1_500_qsub.tsv", append=T, quote=F, sep=" ", row.names=F, col.names=F)		
		}
	}
...
system("less Population_based_regression_endpoint_1_500_qsub.tsv|grep ENDPOINT|awk '{print $1,$2,$3,$5,$6,$7,$8,$9,$11,$12,$13}' > pop_reg_results_1_500_FIN.txt")
