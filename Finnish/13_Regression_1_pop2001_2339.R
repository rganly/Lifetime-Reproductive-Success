...
ep <- ep[ ,c("KANTAHENKILON_TNRO","n_child","n_gchild","SUKULAISEN_SYNTYMAPV","SUKUPUOLI","SYNTYMAKOTIKUNTA","ISCED97","ISCED97.Parent","EduYears","EduYears.Parent",endpoints[2001:2339])]
...
sink(file="pop_reg_selected_endpoint_2001_2339_qsub.output", append=F, type="output", split=T)

#for (ep_n in 1:length(endpoints)){
for (ep_n in 2001:2339){
	...
	for (sex_n in 1:2){	
		...
		if (ep_n==1 & sex_n==1)	{		
			write.table(res, "Population_based_regression_endpoint_2001_2339_qsub.tsv", append=F, quote=F, sep=" ", row.names=F, col.names=T)
		} else {
			write.table(res, "Population_based_regression_endpoint_2001_2339_qsub.tsv", append=T, quote=F, sep=" ", row.names=F, col.names=F)		
		}
	}
...
system("less Population_based_regression_endpoint_2001_2339_qsub.tsv|head -n 1|awk '{print $1,$2,$3,$5,$6,$7,$8,$9,$11,$12,$13}' ")
system("less Population_based_regression_endpoint_2001_2339_qsub.tsv|grep ENDPOINT|awk '{print $1,$2,$3,$5,$6,$7,$8,$9,$11,$12,$13}' > pop_reg_results_2001_2339_FIN.txt")
