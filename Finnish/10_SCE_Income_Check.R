...
sce_inc <- get(load(pate0(r_dir, "sce_inc.Rdata")))
edu_high <- get(load(pate0(r_dir, "edu_high.Rdata")))
...
mean(sce_inc[sce_inc$sose=="","b_year"])

# Cell with highest income in statistical year 2017
min(sce_inc[sce_inc$vuosi==2017,"Eur"],na.rm=T)
max(sce_inc[sce_inc$vuosi==2017,"Eur"],na.rm=T)
median(sce_inc[sce_inc$vuosi==2017,"Eur"],na.rm=T)
mean(sce_inc[sce_inc$vuosi==2017,"Eur"],na.rm=T)
