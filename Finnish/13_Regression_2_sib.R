...
ep <- ep[ ,c("KANTAHENKILON_TNRO","n_child","n_gchild","SUKULAISEN_SYNTYMAPV","SUKUPUOLI","SYNTYMAKOTIKUNTA","ISCED97","ISCED97.Parent","EduYears","EduYears.Parent",endpoints[1:2339])]
...
sink(file="popsib_reg_selected_endpoint_1_2339.output", append=F, type="output", split=T)
...
for (ep_n in 1:length(endpoints)){
	...
	for (sex_n in 1:2){	
		...
		if (ep_n==1 & sex_n==1)	{		
			write.table(res, "popsib_reg_selected_endpoint_1_2339.tsv", append=F, quote=F, sep=" ", row.names=F, col.names=T)
		} else {
			write.table(res, "popsib_reg_selected_endpoint_1_2339.tsv", append=T, quote=F, sep=" ", row.names=F, col.names=F)		
		}
	}
