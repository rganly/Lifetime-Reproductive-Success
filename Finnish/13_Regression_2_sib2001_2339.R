...
sink(file="popsib_reg_selected_endpoint_2001_2339.output", append=T, type="output", split=T)
...
for (ep_n in 2001:2339){
	...
	for (sex_n in 1:2){	
		...
		if (ep_n==1 & sex_n==1)	{		
			write.table(res, "popsib_reg_selected_endpoint_2001_2339_qsub.tsv", append=F, quote=F, sep=" ", row.names=F, col.names=T)
		} else {
			write.table(res, "popsib_reg_selected_endpoint_2001_2339_qsub.tsv", append=T, quote=F, sep=" ", row.names=F, col.names=F)		
		}
	}
