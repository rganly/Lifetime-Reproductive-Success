...
# men born in 1956-1968, women in 1956-1973
indexQC <- indexW %>% filter((index_sex==1 & index_birth_date<=19681231)|(index_sex==2 & index_birth_date<=19731231)) %>% 
      mutate(index_birth_date=as.numeric(as.character(index_birth_date)), index_death_date=as.numeric(as.character(index_death_date))) %>% 
      mutate(endfollowup_date=ifelse(index_sex==1, index_birth_date+500000, index_birth_date+450000)) %>% 
      mutate(endfollowup_date=ifelse(!is.na(index_death_date) & index_death_date<endfollowup_date, index_death_date, endfollowup_date)) %>% 
      select(index_id, index_sex, index_birth_date, index_death_date, endfollowup_date, mother_tongue, birth_town, birth_town_name, birth_municipality, birth_municipality_name)


# birth year 1956-1982, same-sex full-siblings
dup_parent <- indexW %>% filter(!is.na(father_id) & !is.na(mother_id)) %>% 
      mutate(parent_id=paste0(father_id,"_",mother_id,"_",index_sex)) %>% 
      group_by(parent_id) %>% count() %>% filter(n>1)

indexW_fullsib <- indexW %>% filter(!is.na(father_id) & !is.na(mother_id)) %>% 
      mutate(parent_id=paste0(father_id,"_",mother_id,"_",index_sex)) %>% 
      filter(parent_id %in% dup_parent$parent_id) %>% 
...
indexW_fullsib <- indexW_fullsib %>% mutate(age_at_endfollowup=as.numeric(as.Date(as.character(endfollowup_date),"%Y%m%d") - as.Date(as.character(index_birth_date),"%Y%m%d"))/365.25) %>% 
      ...
      mutate(endfollowup4550_age=round(endfollowup4550_age,1)) %>% 
...
table(indexQC$index_sex) # 495,678 males and 607,078 females
table(indexW_fullsib$index_sex)  # 444,033 males and 405,589 females
indexW_fullsib %>% select(index_sex, parent_id) %>% unique() %>% group_by(index_sex) %>% count() %>% rename(N_family="n")  # 195,454 male full-sib family, and 178,512 male full-sib family


save(indexW, file=paste0(r_dir, "IndexW.Rdata"))
save(indexQC, file=paste0(r_dir, "IndexQC.Rdata"))
# save(indexW_fullsib, file=paste0(r_dir, "indexW_fullsib.Rdata"))
...
# Children of index person
indexW_fullsib_child <- child %>% rename(index_id="KANTAHENKILON_TNRO", child_id="SUKULAISEN_TNRO") %>%
      select(index_id, child_id) %>% unique() %>% 
      inner_join(indexW_fullsib[,c("index_id","parent_id","index_birth_date","index_death_date","endfollowup4550_date")], by="index_id") %>% 
...
indexW_to_remove <- indexW_fullsib_child %>% filter((child_birth_date<index_birth_date)|(child_birth_date>index_death_date & !is.na(index_death_date))) %>% select(index_id)
family_to_remove <- indexW_fullsib %>% filter(index_id %!in% indexW_to_remove$index_id) %>% group_by(parent_id) %>% count() %>% filter(n<2)
indexW_fullsib <- indexW_fullsib %>% filter(parent_id %!in% family_to_remove$parent_id)
# save(indexW_fullsib, file=paste0(r_dir, "indexW_fullsib.Rdata"))

indexW_fullsib_child <- indexW_fullsib_child %>% filter(child_birth_date<=endfollowup4550_date) %>% 
                        mutate(index_birth_age=round(as.numeric(as.Date(as.character(child_birth_date),"%Y%m%d") - as.Date(as.character(index_birth_date),"%Y%m%d"))/365.25,1)) %>% 
                        # mutate(index_birth_age=round(age_at_birth/10000,1)) %>% 
...
indexW_fullsib_child <- indexW_fullsib_child %>% mutate(child_id=as.character(child_id)) %>% 
...
dim(indexW_fullsib_child)  # 1567689       11
save(indexW_fullsib_child, file=paste0(r_dir, "indexW_fullsib_child.Rdata"))

table(indexW_fullsib_child$child_death_before28days)
#       0       1 
# 1563763    3926 

table(indexW_fullsib_child$child_ART)
#       0       1 
# 1335106   13827 

table(indexW_fullsib_child$child_preterm)
#       0       1 
# 1269941   78992 
...
indexW_fullsib_marriage <- marriage %>% filter(TUTKHENK_TNRO %in% indexW_fullsib$index_id) %>% 
...
nrow(indexW_fullsib)  #  849,284     
nrow(indexW_fullsib_marriage)  # 705,873
unique(indexW_fullsib_marriage$index_id) %>% length()  # 607,195
save(indexW_fullsib_marriage, file=paste0(r_dir, "indexW_fullsib_marriage.Rdata"))
...
# remove endpoint after the end of 2018
ry_first_indexW <- ry_first %>% filter(ID %in% indexW$ID) %>% filter(ENDPOINT %in% res_ep_info$Endpoint)
ry_first_indexQC <- ry_first %>% filter(ID %in% indexQC$ID) %>% filter(ENDPOINT %in% res_ep_info$Endpoint)
indexW_fullsib_endpoint <- ry_first %>% filter(ID %in% indexW_fullsib$index_id) %>% filter(ENDPOINT %in% res_ep_info$Endpoint) %>%
                                        mutate(EVENT_F_DATE=as.numeric(as.character(EVENT_F_DATE))) %>% filter(EVENT_F_DATE>=19560101 & EVENT_F_DATE<=20181231)

## Need to further editing endpoints with age onset before 16 and after end of follow-up!!!!
nrow(ry_first_indexW)   # 11,279,259
nrow(ry_first_indexQC)  #  7,586,066
nrow(indexW_fullsib_endpoint)   # 5,283,798


save(ry_first_indexW, file=paste0(r_dir, "Endpoint_indexW.Rdata"))
save(ry_first_indexQC, file=paste0(r_dir, "Endpoint_indexQC.Rdata"))
save(indexW_fullsib_endpoint, file=paste0(r_dir, "indexW_fullsib_endpoint.Rdata"))
...
wfile <- c("indexW_fullsib.Rdata", "indexW_fullsib_marriage.Rdata", "indexW_fullsib_child.Rdata", "indexW_fullsib_endpoint.Rdata")
for (k in wfile){
